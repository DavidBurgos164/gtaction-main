    ...
    'render': () => {
        if (mp.players.local.vehicle && mp.players.local.vehicle.getVariable('engineStatus') == false && mp.players.local.vehicle.getPedInSeat(-1) === mp.players.local.handle) {
            if (mp.players.local.vehicle.getVariable('isStalled')) {
                ...
                mp.game.graphics.drawText(`El motor del vehiculo esta apagado, usa ~y~Y~w~ para arrancarlo.`, [0.5, 0.92], {
                    font: 4,
                    color: [255, 255, 255, 170],
                    scale: [0.4, 0.4],
                    outline: false
...
mp.keys.bind(89, false, async function () {
    ...
    if (mp.players.local.vehicle.getVariable('beingRepaired')) return mp.events.call('notifCreate', '~w~El vehiculo se esta ~r~reparando~w~ no puedo arrancarlo.')
    if (!mp.players.local.vehicle.getVariable('engineStatus') && mp.players.local.vehicle.getPedInSeat(-1) === mp.players.local.handle) {
        ...
        mp.events.call('ameCreate', `Intenta encender el motor de ${getVehName(mp.players.local.vehicle.model)}.`);
        ...
        if (Math.floor(Math.trunc(mp.players.local.vehicle.getBodyHealth() / 10)) < 15) {
            return mp.events.call('requestBrowser', `gui.notify.showNotification("El vehiculo esta completamente dañado", false, true, 7000, 'fa-solid fa-triangle-exclamation')`)
        }
        mp.events.call('ameCreate', `Enciende el motor de ${getVehName(mp.players.local.vehicle.model)}.`);
        mp.events.callRemote('engine:start');
    ...
    if (mp.players.local.vehicle.getVariable('engineStatus') && mp.players.local.vehicle.getPedInSeat(-1) === mp.players.local.handle) {
        ...
        mp.events.call('ameCreate', `Apaga el motor de ${getVehName(mp.players.local.vehicle.model)}.`);
        mp.events.callRemote('engine:stop');
    }
});
