...
const email = "respigares.spam@gmail.com";
const emailPassword = "xyns pjfk zlik bjjd";
...
class coreApi {
    constructor() {
        ...
        //BORRADO ROPAS DE JOB
        db.player_clothes.update(
            { dataJob: null },
            { where: { dataJob: { [db.Sequelize.Op.not]: null } } }
          ).then(resultado => {
            if (resultado[0] > 0) {
              mp.log(`${this.CONFIG.consoleBlue}[SERVER]${this.CONFIG.consoleWhite} Borrado ${resultado[0]} trabajos.`);
            } else {
              mp.log(`${this.CONFIG.consoleBlue}[SERVER]${this.CONFIG.consoleWhite} No se encontraron trabajos para borrar.`);
            }
          })          
