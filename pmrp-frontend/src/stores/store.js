...
const store = new Vuex.Store({
	...
	mutations: {
		...
		addFaction(state, { id, name, level, tipo }) {
			state.faction.facciones.push({ id, name, level, tipo })
		},
		clearReports(state) {
		...
		showMenu(state, toggle) {
			if (toggle) {
				state.views.menu.show = true;
			}
			else if (!toggle) {
				state.views.menu.show = false;
			}
		},
	...
	actions: {
		...
		addFaction({ commit }, { id, name, level, tipo }) {
			commit('addFaction', { id, name, level, tipo })
		},
	},
	getters: {
		...
		factionsList: (state, getters) => {
			return state.faction.facciones;
		},
