        ...
        <div class="container-charSelect">
          <div v-if="charNames.length > 0" class="selectionBase" style="margin-left:2vw; position:absolute; height:14vw;">
            ...
            <div style="line-height:2vw; padding-bottom:1vw;">
              <p style="margin-top:0.5vw; color:rgba(255, 255, 255, 0.576); margin-left:0.3vw; font-size:0.9vw;">Codigo de referencia <font style="float:right; margin-right:0.6vw; color:white;">{{ charNames[0].referral }}</font></p>
              <p style="margin-top:0.5vw; color:rgba(255, 255, 255, 0.576); margin-left:0.3vw; font-size:0.9vw;">Fecha de creacion <font style="float:right; margin-right:0.6vw; color:white; font-size:0.9vw;">{{ cut(charNames[0].creation) }}</font></p>
              <p style="margin-top:0.5vw; color:rgba(255, 255, 255, 0.576); margin-left:0.3vw; font-size:0.9vw;">Total de horas jugadas <font style="float:right; margin-right:0.6vw; color:white; word-wrap: break-word;">{{ formatNum(charNames[0].totalHours) }}</font></p>
              <p style="margin-top:0.5vw; color:rgba(255, 255, 255, 0.576); margin-left:0.3vw; font-size:0.9vw;">Dinero total <font style="float:right; margin-right:0.6vw; color:rgba(255, 223, 63, 0.835); word-wrap: break-word;">${{ formatNum(charNames[0].adminPunishments) }}</font></p>
            </div>
          </div>
          <div class="selectionBase" style="margin-left:2vw; position:absolute; height:33vw; margin-top:17vw; background-color:transparent; background-image: url('...'); border-bottom: none;">
            <div class="headerSelect" style="background-color: #000000b1; background-image: url('./assets/image/diagmonds.png');"><i class="fa-solid fa-envelope"></i> Notificaciones <p style="margin-top:0.3vw">Notificaciones relacionadas con tu cuenta y administración. </p></div>
            <div style="padding-bottom:1vw; overflow:scroll; height:41vw; overflow-x: hidden;">
              ...
              <div v-else>
                <div style="background-color:rgba(158, 158, 158, 0.305); padding:0.5vw; margin-top:0.5vw; margin-left:0.2vw; background-color: #0000009a; background-image: url('./assets/image/diagmonds.png'); border-bottom: solid grey 3px;">
                  <p style="color:rgb(255, 255, 255); margin-left:0.3vw; font-size:0.9vw; word-wrap:break-word;">No tienes notificaciones pendientes.<font style="float:right; margin-right:0.6vw; color:white;"></font></p>
                </div>
              </div>
            </div>
          </div>
        <div class="selectionBase" style="background-color:transparent; background-image:none;">
            <div class="headerSelect" style="background-color:rgba(1, 1, 1, 0.7);"><i class="fa-solid fa-user-group"></i> Seleccion de personaje
              <p v-if="charNames[0].name" style="margin-top:0.3vw;">Selecciona tu personaje {{ charNames.length }} {{ charNames.length == 0 ? `/ 2` : `/ ${charNames[0].max}` }}</p>
              <p v-else style="margin-top:0.4vw;">No tienes personajes. Haz clic en el botón Crear personaje para crear uno.</p>
            </div>
                    ...
                    <table class="tableOne" cellspacing="2" style="table-layout:fixed; text-align: left; margin-left: 0.1vw;">
                        <tr>
                          <th v-if="charNames.length > 0">
                            <i class="fa-solid fa-user"></i> Nombre: <b style="color:grey">{{ formatName(cName.name) }}</b><br>
                            <i class="fa-solid fa-clock"></i> Tiempo de juego: <b style="color:grey">{{ cName.hours }} days</b><br>
                            <i class="fa-solid fa-building-columns"></i> Banco: <b style="color:grey">{{ '$'+cName.bank }}</b><br>
                        </th>
                        <th v-if="charNames.length > 0" style="margin-right: 2vw;">
                            <i class="fa-sharp fa-solid fa-people-group" style="margin-left:0.3vw;"></i> Faccion: <b style="color:grey">{{ cName.faction }}</b><br>
                            <i class="fa-solid fa-notes-medical" style="margin-left:0.3vw;"></i> Salud: <b style="color:grey">{{ cName.hp }}</b><br>
                            <i class="fa-solid fa-right-to-bracket" style="margin-left:0.3vw;"></i> Ultima vez: <b style="color:grey">{{ formatUnixTimestamp(cName.lastPlayed) }}</b><br>
                        </th>
                        </tr>
                          <tr>
            ...
            <b v-if="charNames.length==0 || !charNames[0].name"><div class="headSelect" style="color:grey; font-weight: 500;">No tienes ningun personaje.</div></b>
          </div>
        <button class="createCharBtn" style="margin-top:1vw;" @click="goToCharCreation()">Crear personaje</button>
      </div>
    </div>
</template>
